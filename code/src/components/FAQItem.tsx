...
export function FAQItem({ question, answer, index }: FAQItemProps) {
  ...
  const toggle = () => {
    ...
    if (!open) {
      ...
      gsap.fromTo(content, { height: 0, opacity: 0 }, { height: h, opacity: 1, duration: 0.45, ease: 'power3.out' });
    ...
    <div className="single-reveal border-b border-[#F7E9E8]/10">
      ...
      >
        <div className="flex items-center gap-4 sm:gap-6">
          ...
          <span className="font-grotesk font-semibold text-lg sm:text-xl text-[#F7E9E8] group-hover:text-[#AD1D12] transition-colors duration-200">
            {question}
          </span>
        </div>
        <ChevronDown ref={arrowRef as React.Ref<SVGSVGElement>} className="w-5 h-5 text-[#F7E9E8]/40 shrink-0" />
      </button>
